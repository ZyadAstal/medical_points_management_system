...
<body>
    ...
    <main class="auth-page" aria-label="صفحة استعادة كلمة المرور">
        <section class="auth-frame" aria-label="إطار استعادة كلمة المرور">
            ...
            <p class="auth-welcome-sub">أدخل بريدك الإلكتروني لتلقي رابط إرسال كلمة المرور</p>
            ...
            <form class="auth-form" method="POST" action="{{ route('password.email') }}" autocomplete="off">
                ...
                <input
                    id="email"
                    name="email"
                    class="field-input field-input-email @error('email') is-invalid @enderror"
                    type="email"
                ...
                />
                @error('email')
                    <span class="invalid-feedback" role="alert" style="color: #dc3545; font-size: 14px; margin-top: 5px; display: block;">
                        <strong>{{ $message }}</strong>
                    </span>
                @enderror

                <button class="auth-submit" type="submit" style="margin-top: 24px;">إرسال رابط الاستعادة</button>
            </form>
